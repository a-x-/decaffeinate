...
var ProgramPatcher_1 = require("./ProgramPatcher");
...
var ClassPatcher = /** @class */ (function (_super) {
    ...
    ClassPatcher.prototype.patchAsStatement = function () {
        ...
        this.addSuggestion(suggestions_1.AVOID_INITCLASS);
        ...
        if (shouldUseIIFE) {
            ...
            if (this.nameAssignee instanceof IdentifierPatcher_1.default) {
    ...
    ClassPatcher.prototype.shouldUseIIFE = function () {
        ...
        if (this.parent instanceof BlockPatcher_1.default) {
            var statements = this.parent.statements;
            if (!(this.parent.parent instanceof ProgramPatcher_1.default) && this === statements[statements.length - 1]) {
                return true;
            }
            return false;
        }
    ...
    ClassPatcher.prototype.isClassMethod = function (patcher) {
        if (patcher instanceof ConstructorPatcher_1.default) {
            return true;
        }
        ...
        if (this.isClassAssignment(node)) {
            ...
            // bound.
            if (node instanceof nodes_1.AssignOp && ['BoundFunction', 'BoundGeneratorFunction'].indexOf(node.expression.type) >= 0) {
                return false;
            }
            if (node.expression instanceof nodes_1.BaseFunction) {
                return true;
            }
        }
        return false;
    ...
    ClassPatcher.prototype.isClassAssignment = function (node) {
        if (node instanceof nodes_1.ClassProtoAssignOp) {
            return true;
        }
        if (node instanceof nodes_1.AssignOp) {
            var assignee = node.assignee;
            if (assignee instanceof nodes_1.MemberAccessOp || assignee instanceof nodes_1.DynamicMemberAccessOp) {
                if (assignee.expression instanceof nodes_1.This) {
                    return true;
                }
                if (this.nameAssignee && this.nameAssignee instanceof IdentifierPatcher_1.default) {
                    var className = this.nameAssignee.node.data;
                    if (assignee.expression instanceof nodes_1.Identifier && assignee.expression.data === className) {
                        return true;
                    }
                }
            }
    ...
    ClassPatcher.prototype.needsCustomConstructor = function () {
        ...
        try {
            for (var _b = tslib_1.__values(this.body.statements), _c = _b.next(); !_c.done; _c = _b.next()) {
                var patcher = _c.value;
                if (patcher instanceof ConstructorPatcher_1.default && !(patcher.expression instanceof FunctionPatcher_1.default)) {
                    return true;
                }
            }
        }
    ...
    ClassPatcher.prototype.extractCustomConstructorInfo = function () {
        ...
        try {
            for (var _b = tslib_1.__values(this.body.statements), _c = _b.next(); !_c.done; _c = _b.next()) {
                var patcher = _c.value;
                if (patcher instanceof ConstructorPatcher_1.default) {
                    if (!(patcher.expression instanceof FunctionPatcher_1.default)) {
    ...
    ClassPatcher.prototype.getAssignmentName = function (statementPatcher) {
        if (statementPatcher.node instanceof nodes_1.AssignOp && statementPatcher.node.assignee instanceof nodes_1.Identifier) {
            return statementPatcher.node.assignee.data;
        }
        if (statementPatcher instanceof ClassPatcher && statementPatcher.nameAssignee instanceof IdentifierPatcher_1.default) {
